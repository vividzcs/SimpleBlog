...
<body>
    ...
    <div class="crumb_warp">
        ...
        <i class="fa fa-home"></i> <a href="{{url('admin/info')}}">首页</a>  &raquo; 编辑栏目
    </div>
    <!--面包屑导航 结束-->
    ...
    <div class="result_wrap">
        <form action="{{url('admin/category/' . $cate->cate_id)}}" method="post">
            ...
            <table class="add_tab">
                <tbody>
                    <tr>
                        <th width="120"><i class="require">*</i>添加分类：</th>
                        <td>
                            <select name="cate_pid">
                                <option value="">==顶级分类==</option>
                                @foreach($cates as $d)
                                <option value="{{$d['cate_id']}}"
                                    @if($d->cate_id == $cate->cate_pid)
                                        selected
                                    @endif
                                >{{str_repeat('&nbsp;&nbsp;',$d['lev']) . $d['cate_name']}}</option>
                                @endforeach
                            </select>
                        </td>
                    </tr>
